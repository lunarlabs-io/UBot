...
module.exports = class PrefixCommand extends Command {
	...
	async run(msg, args) {
		...
		if(lowercase === 'default') {
			...
			if(args.prefix != null || msg.guild) {
                var sql = `UPDATE serversettings SET prefix = '${args.prefix}' WHERE serverID = '${msg.guild.id}';`
                global.con.query(sql, function (err, result) {
                    if (err) throw err;
                    console.log("[Prefix] Server prefix changed!");
                    msg.guild.commandPrefix = args.prefix
                msg.channel.send("Prefix set to: " + args.prefix)
                  });
            }
		}
